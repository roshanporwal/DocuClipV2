      ...
      .then((subCategories) => {
        ...
        if (typeof props.metadata.additional_info === "object") {
          ...
          for (const property in props.metadata.additional_info) {
            ...
            if (
              value.match(regexOne) ||
              value.match(regexTwo) ||
              value.match(regexThree)
            ) {
              if (!changedDateOnceFlag) index = 4;
  ...
  const shareClickHandler = async () => {
    ...
    if (props.metadata.is_local == "true") {
      const uniqueFilename =
        props.publicName.substr(0, 8) + "_" + props.fileName;
      ...
      Filesystem.readFile({
        ...
        .then((data: { data: any }) => {
          ...
          Filesystem.getUri({
            directory: FilesystemDirectory.Documents,
            path: "docuclip/" + uniqueFilename,
          }).then(async (getUriResult: { uri: any }) => {
            const path = getUriResult.uri;
            FilePath.resolveNativePath(path).then((filePath) => {
          ...
          <div className="file-info">
            {props.showCategoryTitle === true ? (
              <span className="heading">
                <b>{infoArray[0]}</b>
                {typeof infoArray[1] !== "undefined" ? (
                  <React.Fragment> /{infoArray[1]}</React.Fragment>
                ) : null}
              </span>
            ) : null}
